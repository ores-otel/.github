...
3. Review linked pull requests, issues, Linear work, related repositories in `ores-otel`, and relevant external-organization repositories whenever behavior or contracts cross boundaries.
...
- Keep `tmp/`, `temp/`, `tmp/worktrees`, and `temp/worktrees` ignored in the repository-root `.gitignore`. Use the directory-only `tmp/` and `temp/` forms required by the DEN-3956 `scratch-ignore-policy` check; a bare `tmp` or `temp` rule would also hide legitimate files with those names. Do not commit files from those directories.
